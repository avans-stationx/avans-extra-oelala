...
// DMAChannel dma;
// volatile uint8_t dmaTxBuffer[256];
// #endif
...
const size_t UPDATE_BYTE_AMOUNT = 32;
const size_t WRITE_BURST_LENGTH = 256;

// Constructor
Adafruit_Thermal::Adafruit_Thermal(Stream* s, size_t bufferSize, uint8_t dtr)
    : stream(s), buffer(bufferSize), dtrPin(dtr) {
  dtrEnabled = false;
  writeBurst = 0;
  nextWriteAfter = millis();
}
...
void Adafruit_Thermal::begin(uint16_t version) {
  ...
  // #if defined(ARDUINO_TEENSY41)
  //   dma.begin();
  //   dma.source((uint8_t&)dmaTxBuffer);
  // #endif
...
void Adafruit_Thermal::update() {
  if (millis() < nextWriteAfter) {
    return;
  }

  // if ((long)(micros() - resumeTime) < 0L) {
  //   return;
  // }

  // while (buffer.readable() && writeBurst < WRITE_BURST_LENGTH) {
  //   stream->write(buffer.pop());
  //   writeBurst += 1;
  // }

  if (buffer.readable()) {
    size_t maxLength = WRITE_BURST_LENGTH - writeBurst;
    size_t readableLength =
        maxLength > UPDATE_BYTE_AMOUNT ? UPDATE_BYTE_AMOUNT : maxLength;
    uint8_t bytes[readableLength];
    size_t readLength = buffer.popMany(bytes, readableLength);

    stream->write(bytes, readLength);

    writeBurst += readLength;
  }

  if (writeBurst == WRITE_BURST_LENGTH) {
    // delay(1);

    nextWriteAfter = millis() + 1;
    writeBurst = 0;
  }
}
...
#define DOUBLE_HEIGHT_MASK \
  (1 << 4)                          //!< Turn on/off double-height printing mode
...
void Adafruit_Thermal::setHeatConfig(uint8_t dots,
                                     uint8_t time,
                                     uint8_t interval) {
...
void Adafruit_Thermal::printBitmap(int w,
  ...
  for (y = 0; y < h; y++) {
    for (x = 0; x < rowBytesClipped; x++, i++) {
      ...
      //   for (x = 0; x < rowBytesClipped; x++, i++) {
      // timeoutWait();
      // stream->write(fromProgMem ? pgm_read_byte(bitmap + i) : *(bitmap +
      // i));
      buffer.unshift(fromProgMem ? pgm_read_byte(bitmap + i) : *(bitmap + i));
    }
